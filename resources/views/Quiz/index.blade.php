...
<body class="bg-[#F3F2EC] font-['Manrope',sans-serif]">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="text-black">
                                ...
                                <th class="p-5 text-left text-xs font-semibold uppercase tracking-wide">Nilai Lulus</th>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @forelse($quizzes as $i => $quiz)
                                <tr
                                    ...
                                    <td class="p-5 pl-8 text-sm text-gray-500">{{ $i + 1 }}</td>
                                    <td class="p-5 text-sm font-semibold text-gray-900">{{ $quiz->title }}</td>
                                    <td class="p-5 text-sm">
                                        <span
                                            class="bg-gray-100 text-gray-700 text-xs font-semibold px-2.5 py-1 rounded-lg">{{ $quiz->service->name }}</span>
                                    </td>
                                    <td class="p-5 text-sm text-gray-700">{{ $quiz->duration_minutes }} menit</td>
                                    <td class="p-5 text-sm text-gray-700">{{ $quiz->passing_score }}%</td>
                                    <td class="p-5 text-sm">
                                        @if ($quiz->is_active)
                                            <span
                                    ...
                                    <td class="p-5 pr-8">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('quiz.show', $quiz) }}"
                                                class="w-9 h-9 flex items-center justify-center text-blue-500 hover:bg-blue-50 rounded-lg transition-all hover:scale-110 border border-transparent hover:border-blue-200"
                                                title="Detail">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="py-12 text-center text-slate-400 text-lg">
                                        Belum ada quiz.
                                    </td>
                                </tr>
                            @endforelse
    ...
    <div id="addModal"
        ...
        <div
            class="relative w-[90%] max-w-[560px] max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-[0_32px_64px_rgba(0,0,0,0.2)]">
            {{-- Header --}}
            <div class="relative bg-black px-8 py-6">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none"
                            stroke="currentColor" stroke-width="2.5" class="text-white" viewBox="0 0 24 24">
                            <line x1="12" y1="5" x2="12" y2="19" />
                            <line x1="5" y1="12" x2="19" y2="12" />
                        </svg>
                    </div>
                    <div>
                        ...
                        <p class="text-white/60 text-xs mt-0.5">Buat quiz baru untuk Service</p>
                    </div>
                </div>
            ...
            <form action="{{ route('quiz.store') }}" method="POST" id="addQuizForm" class="px-8 py-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Deskripsi
                        (opsional)</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Durasi
                            (menit)</label>
                        <input type="number" name="duration_minutes" value="30" min="1" required
                            class="w-full h-11 bg-slate-50 border border-slate-200 rounded-xl px-4 text-sm text-slate-700 font-medium focus:bg-white focus:border-[#E62727] focus:ring-4 focus:ring-[#E62727]/10 outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Nilai
                            Lulus (%)</label>
                    ...
                    <div id="questionList" class="flex flex-col gap-3">
                        <div class="question-item bg-slate-50 border border-slate-200 rounded-xl p-4">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <button type="button"
                                    class="remove-question text-red-400 hover:text-red-600 transition-colors hidden">
                            ...
                            <div class="flex items-center gap-2">
                                <label
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Poin:</label>
                                <input type="number" name="questions[0][score]" value="10" min="1"
                                    required
                ...
                <div class="flex gap-3 pt-4">
                    <button type="button" onclick="closeAddModal()"
                        class="flex-1 h-11 bg-slate-100 text-slate-500 rounded-xl text-sm font-bold hover:bg-slate-200 transition-all">Batal</button>
                    <button type="submit"
                        class="flex-[2] h-11 bg-gradient-to-r from-[#8B0000] to-[#E62727] text-white rounded-xl text-sm font-bold shadow-[0_4px_15px_rgba(230,39,39,0.3)] hover:scale-[1.02] active:scale-95 transition-all">Simpan
                        Quiz</button>
                </div>
            </form>
        </div>
            ...
            <form id="editForm" method="POST" class="px-8 py-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Nilai
                            Lulus (%)</label>
                        <input type="number" id="edit_passing" name="passing_score" min="0" max="100"
                            required
                            class="w-full h-11 bg-slate-50 border border-slate-200 rounded-xl px-4 text-sm text-slate-700 font-medium focus:bg-white focus:border-[#E62727] focus:ring-4 focus:ring-[#E62727]/10 outline-none">
                    </div>
                </div>
                <div>
                    <label
                        class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Status</label>
