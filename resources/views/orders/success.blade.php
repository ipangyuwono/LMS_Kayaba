        ...
        <div class="w-full max-w-md">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                @if ($order && $order->status === 'paid')
                    <div
                        class="relative bg-gradient-to-br from-emerald-500 to-teal-500 px-6 pt-10 pb-14 text-center overflow-hidden">
                        ...
                        <div class="relative">
                            <div
                                class="w-16 h-16 bg-white/20 backdrop-blur rounded-2xl flex items-center justify-center mx-auto mb-4 border border-white/30">
                ...
                @else
                    <div
                        class="relative bg-gradient-to-br from-amber-400 to-orange-500 px-6 pt-10 pb-14 text-center overflow-hidden">
                        <div class="absolute inset-0 opacity-10">
                            <div class="absolute top-2 left-8 w-24 h-24 rounded-full bg-white"></div>
                            <div class="absolute bottom-0 right-4 w-32 h-32 rounded-full bg-white"></div>
                        </div>
                ...
                <div class="relative -mt-5 flex justify-center">
                    @if ($order && $order->status === 'paid')
                        <span
                            class="inline-flex items-center gap-1.5 bg-white border border-emerald-100 text-emerald-600 text-xs font-bold px-4 py-1.5 rounded-full shadow-sm">
                            <span class="w-1.5 h-1.5 bg-emerald-500 rounded-full"></span>
                            PAID
                        </span>
                    @elseif($order && $order->status === 'pending')
                        <span
                            class="inline-flex items-center gap-1.5 bg-white border border-amber-100 text-amber-600 text-xs font-bold px-4 py-1.5 rounded-full shadow-sm">
                            <span class="w-1.5 h-1.5 bg-amber-400 rounded-full animate-pulse"></span>
                            PENDING
                        </span>
                    @else
                        <span
                            class="inline-flex items-center gap-1.5 bg-white border border-red-100 text-red-600 text-xs font-bold px-4 py-1.5 rounded-full shadow-sm">
                ...
                @if ($order && $order->status === 'pending')
                    <div class="mx-6 mt-4">
                        <div id="countdown-wrap"
                            class="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3">
                            <div class="w-8 h-8 rounded-xl bg-amber-100 flex items-center justify-center shrink-0">
                                <svg id="countdown-icon" width="15" height="15" viewBox="0 0 24 24" fill="none"
                            ...
                            <div class="flex-1 min-w-0">
                                <p id="countdown-label" class="text-xs font-semibold text-amber-800">Batas waktu pembayaran
                                </p>
                                <p class="text-xs text-amber-600 mt-0.5">Selesaikan sebelum waktu habis</p>
                            </div>
                            <span id="countdown-display"
                                class="font-mono text-sm font-bold text-amber-700 bg-amber-100 px-3 py-1.5 rounded-xl shrink-0 tabular-nums">
                                --:--:--
                            </span>
                        </div>
                    </div>
                ...
                @if ($order)
                    <div class="mx-6 mt-5 bg-slate-50 rounded-2xl overflow-hidden">
                        ...
                        <div class="flex justify-between items-center px-4 py-3">
                            ...
                            <span
                                class="font-mono text-xs bg-white border border-slate-200 text-slate-500 px-2.5 py-1 rounded-lg">{{ $order->order_id }}</span>
                        </div>
                    </div>
                @endif
                ...
                <div class="px-6 py-6 flex flex-col gap-2.5">
                    ...
                    @if ($order && $order->status === 'paid')
                        <a href="{{ route('orders.invoice', $order) }}"
                            class="w-full text-center bg-gradient-to-r from-[#8B0000] to-[#E62727] hover:opacity-90 active:scale-95 text-white text-sm font-bold py-3.5 rounded-2xl transition-all duration-150 shadow-sm hover:shadow-md flex items-center justify-center gap-2">
                            Download Invoice
                        </a>
                    @endif
