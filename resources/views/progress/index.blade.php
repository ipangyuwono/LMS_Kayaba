                ...
                <div
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-0.5">Total materi aktif: <span
                                    class="font-bold text-gray-600">{{ $totalMaterials }}</span></p>
                        </div>
                    </div>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($customers as $cust)
                                @php
                                    $pct =
                                        $totalMaterials > 0
                                            ? round(($cust->completed_count / $totalMaterials) * 100)
                                            : 0;
                                    $barColor =
                                        $pct >= 100 ? 'bg-green-500' : ($pct > 0 ? 'bg-[#E62727]' : 'bg-gray-300');
                                @endphp
                                <tr
                                    ...
                                    <td class="p-5">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span
                                                class="text-xs font-bold text-gray-600 min-w-[3rem] text-right">{{ $cust->completed_count }}/{{ $totalMaterials }}</span>
                                        </div>
                                        <div class="flex items-center gap-2 mt-1.5">
                                            @if ($cust->in_progress_count > 0)
                                                <span
                                                    class="text-[10px] bg-yellow-100 text-yellow-700 font-semibold px-1.5 py-0.5 rounded">{{ $cust->in_progress_count }}
                                                    sedang dipelajari</span>
                                            @endif
                                            @if ($pct >= 100)
                                                <span
                                                    class="text-[10px] bg-green-100 text-green-700 font-semibold px-1.5 py-0.5 rounded">✓
                                                    Selesai</span>
                                            @endif
                                        </div>
                                    </td>
